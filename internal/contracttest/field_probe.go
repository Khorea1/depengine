...
var fieldPairs = map[string][2]any{
	...
	"cargo.secret_ref": {
		map[string]any{"provider": "env", "name": "CARGO_TOKEN_A"},
		map[string]any{"provider": "env", "name": "CARGO_TOKEN_B"},
	},
	"repo":       {"org/a", "org/b"},
...
var baseOverrides = map[string]string{
	"container.secret_ref": "container+auth",
	"cargo.secret_ref":     "cargo+git",
...
var baseConfigs = map[string]map[string]any{
	"container":      {"manager": "docker", "source": "registry.example.test/team/demo"},
