...
Audit local and HTTP archive extraction first. Keep the portable naming,
collision, resource-limit, and ownership checks, then perform creation and
lookup through a root opened on the staging directory. The same approach may
help project-relative vendored reads and future archive installers. Rooted
