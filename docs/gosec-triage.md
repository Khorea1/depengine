...
uncapped scan reports 417 diagnostics: 67 in production and 350 in tests.
...
- Bound local archive expansion in `internal/localartifact/install.go`
  (G110). ZIP extraction currently copies without an application byte budget;
  TAR entry sizes likewise do not impose an aggregate extraction limit.
